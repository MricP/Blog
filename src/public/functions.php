...
function deleteCategory($idcategory) {
    try {
        ...
        $connexion = new PDO('mysql:host='.$GLOBALS['db']['host'].';dbname='.$GLOBALS['db']['name'], 'root', '');
        $connexion->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "DELETE FROM ".$GLOBALS['db']['tables'][2]." WHERE id_category=:idcategory";
...
function modifyCategory ($newCategoryName,$categoryId,&$errorMessage){
    try {
        $connexion = new PDO('mysql:host='.$GLOBALS['db']['host'].';dbname='.$GLOBALS['db']['name'], 'root', '');
        $connexion->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "UPDATE ".$GLOBALS['db']['tables'][2]." SET name_category=:newCategoryName WHERE id_category=:idcategory";
        ...
        $errorMessage = "Modification impossible, cette catégorie existe déjà !";
    } catch (Exception $e) {
        die('Erreur Générale : ' . $e->getMessage());
    }
}

function createCategory ($categoryName,&$errorMessage){
    try {
        $connexion = new PDO('mysql:host='.$GLOBALS['db']['host'].';dbname='.$GLOBALS['db']['name'], 'root', '');
        $connexion->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "INSERT INTO ".$GLOBALS['db']['tables'][2]."(name_category) VALUES (:categoryName)";
        
        $stmt = $connexion->prepare($sql);
        $stmt->bindParam(':categoryName', $categoryName);
        $stmt->execute();
    } catch (PDOException $e) { 
        $errorMessage = "Ajout impossible, cette catégorie existe déjà !";
    } catch (Exception $e) {
        die('Erreur Générale : ' . $e->getMessage());
    }
}
